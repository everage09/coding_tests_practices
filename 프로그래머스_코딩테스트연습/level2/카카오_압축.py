...
def solution(msg):
    answer = []
    base_dict = {letter : index for index,letter 
                 in enumerate("ABCDEFGHIJKLMNOPQRSTUVWXYZ", start=1)}
    w = None
    for idx, letter in enumerate(msg):
        ...
        if wc in base_dict:
            ...
            base_dict[wc] = len(base_dict)+1
            answer.append(base_dict[w])
            w = None
    if not w:
        answer.append(base_dict[msg[-1]])
    else:
        answer.append(base_dict[w+msg[-1]])
    return answer

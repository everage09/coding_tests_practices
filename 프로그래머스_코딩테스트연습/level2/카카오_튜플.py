...
def solution(s):
    answer = []
    # Process s into list of lists
    l = s.split("},{")
    for idx, elem in enumerate(l):
        l[idx] = list(map(int, elem.replace("{","").replace("}","").split(",")))
    l.sort(key=lambda x: len(x))  # -> (key = len) works
    
    # first element of finding tuple
    answer.append(l[0][0])
    # compare the difference of the two sets
    # since the finding tuple has no duplication
    for idx in range(1,len(l)):
        a = (set(l[idx]))
        b = (set(l[idx-1]))
        answer.append(list(a-b)[0])
    return answer

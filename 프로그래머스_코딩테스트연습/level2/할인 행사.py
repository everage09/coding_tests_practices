...
# Slice discount list to check if items are all in.
def solution(want, number, discount):
    answer = 0
    wish = {w : n for w,n in  zip(want,number)}
    total = sum(number)
    for start in range(0,len(discount)-total+1):
        part = discount[start:start+total]
        if any(item not in part for item in want):
            continue
        if all(part.count(item) == wish[item] for item in want):
            answer+=1
    return answer

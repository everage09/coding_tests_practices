...
# Make a keymap dictionary and save minimum number of pushes
def solution(keymap, targets):
    answer = []
    key_dict = dict()
    # make keymap dictionary 
    for key in keymap:
        for idx, alpha in enumerate(key):
            # this part can be compressed into one line:
            # key_dict[alpha] = idx+1 if alpha not in key dict \n
            # else min(key_dict[alpha], idx+1)
            if alpha not in key_dict:
                key_dict[alpha] = idx+1
            else:
                if key_dict[alpha] > idx+1:
                    key_dict[alpha] = idx+1
    # compute answer

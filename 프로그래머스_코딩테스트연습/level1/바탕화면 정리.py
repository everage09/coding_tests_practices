...
# Solved it in a little silly way. -> Could be much more simple

def solution(wallpaper):
    S = [len(wallpaper),len(wallpaper[0])]
    E = [0,0]

    # finding Top-Bottom boundary
    for idx, row in enumerate(wallpaper):
        if "#" in row:
            S[0] = idx
            break
    
    for idx, row in enumerate(wallpaper[::-1]):
        if "#" in row:
            E[0] = len(wallpaper)-idx
            break
    # finding left-right boundary          
    ...
    for row in sorted(wallpaper, reverse=True):
        for idx, elem in enumerate(row[::-1]):
            if elem == "#":
                E[1] = max(E[1], len(row)-idx)
                break           

...
def solution(board, moves):
    ...
    basket = []
    N = len(board)
    # re-positioning lines to think them in verical direction
    newBoard = [[board[N-1-i][j] for i in range(N)] for j in range(N)]
    
    def drop_doll(doll_type):
        nonlocal answer
        nonlocal basket
        if len(basket) != 0:
            if basket[-1] == doll_type:
                basket.pop()
                answer += 2
            else:

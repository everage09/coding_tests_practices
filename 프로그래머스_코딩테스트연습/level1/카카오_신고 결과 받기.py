...
def solution(id_list, report, k):
    ...
    for id in id_list:
        record[id] = {"num_reported" : 0,
                     "report_targets" : [],
                     "num_result" : 0}
    
    for i in range(len(report)):
        reporter, target = report[i].split(' ')
        targets = record[reporter].get("report_targets")
        if target not in targets:
            targets.append(target)
            record[target]["num_reported"] += 1
    
    for idx in range(len(id_list)):
        targets = record[id_list[idx]].get("report_targets")
        for target in targets:
            if record[target]["num_reported"] >= k:
                record[id_list[idx]]["num_result"] += 1
        answer[idx] = record[id_list[idx]]["num_result"]

...
# Use dictionary for score count
def solution(survey, choices):
    answer = ''
    category = {t : 0 for t in ["R","T","C","F","J","M","A","N"]}
    score = [3,2,1,0,1,2,3]
    for q, ans in zip(survey, choices):
        if ans in [1,2,3]:
            category[q[0]] += score[ans-1]
        elif ans in [5,6,7]:
            pass
            category[q[1]] += score[ans-1]
        else:
            continue
    answer += "R" if category["R"] >= category["T"] else "T"
    answer += "C" if category["C"] >= category["F"] else "F"
    answer += "J" if category["J"] >= category["M"] else "M"
    answer += "A" if category["A"] >= category["N"] else "N"
